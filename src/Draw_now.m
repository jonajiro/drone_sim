...
subplot(1,2,1);%上面
hold on
trisurf(F_body,V_body_t(:,1),V_body_t(:,2),V_body_t(:,3),'FaceColor','cyan','EdgeColor','none');
...
%側面
hold on
trisurf(F_body,V_body_t(:,1),V_body_t(:,2),V_body_t(:,3),'FaceColor','red','EdgeColor','none');
plot3(tmp(i,11),tmp(i,12),tmp(i,13),'o');
xlabel('ダウンレンジ[m]')
ylabel('クロスレンジ[m]')
zlabel('高度[m]')
